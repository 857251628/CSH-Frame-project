(function($){var IE6=(window.XMLHttpRequest==undefined)&&(ActiveXObject!=undefined);var IE7=(typeof document.addEventListener!='function'&&window.XMLHttpRequest&&typeof document.querySelector=="undefined")?true:false;var sidebar=function(obj){var $slidebar=obj.event;var $slide=$slidebar.find(".slide");var $slideLen=$slide.length;$slide.each(function(index){$root=$(this);var $slideUL=$root.find('ul');if($.trim($slideUL.html())!=""){if($root.hasClass(obj.cur)){$root.find('.slide_lv2').slideDown();}
$root.click(function(){$this=$(this);$slide_lv2=$this.find('.slide_lv2');if($this.hasClass(obj.cur)){$slide_lv2.slideUp('fast',function(){$this.removeClass(obj.cur);});}else{$this.addClass(obj.cur).siblings().removeClass(obj.cur);$slide_lv2.hide();$slide_lv2.slideDown();}});}else{$root.addClass(obj.active);}});}
...
$thumbs.eq(0).click();});}
var focusMap=function(obj){var $imgRoot=$('#js_focus_image');var $list=$imgRoot.find('.item');var $controler=$imgRoot.find('.controler b');var autoPlay=obj.autoPlay;var flag=obj.autoPlay;var index=0;var autoTime=obj.autoTime;$list.eq(0).addClass(obj.cur);$controler.eq(0).addClass(obj.cur)
var goTo=function(){var $next=$imgRoot.find('.controler b.cur').next();if($next.length){$next.click();}else{$controler.eq(0).click();}}
$controler.click(function(){var $this=$(this);var index=$this.index();var item=$list.eq(index);var img=item.find('.poster');img.attr("src",img.attr('data-src'));item.fadeIn().addClass('cur').siblings().fadeOut().removeClass('cur');$this.addClass('cur').siblings().removeClass('cur');}).hover(function(){if(autoPlay){flag=false;}},function(){if(autoPlay){flag=true;}});$list.hover(function(){if(autoPlay){flag=false;}},function(){if(autoPlay){flag=true;}});window.setInterval(function(){if(flag){goTo()}},autoTime);}
...
hideButton();});};var xmlData=null;var pageIndexTpl=1;var Page=function(){var $this=this;this._start=1;this._end=1;this._pageIncludeTpl=5;this._makeUrl=function(obj){var _dataUrl=obj.dataUrl;var _pageIndex=obj.pageIndex-1;if(_pageIndex>0){_dataUrl=_dataUrl.replace(".xml",'_'+_pageIndex+'.xml');_dataUrl=_dataUrl.replace(_dataUrl.substring(_dataUrl.indexOf("_")),'_'+_pageIndex+'.xml');return _dataUrl;}else{return _dataUrl;}
return obj.dataUrl.replace(dataUrl.substring(dataUrl.indexOf("_")),(obj.pageIndex-1)>0?'_'+(obj.pageIndex-1)+'.xml':'.xml');}
this._showData=function(obj){$(obj.pageData).html("")
for(var i=0;i<xmlData.length;i++){var strHTML=obj.ajaxData(xmlData[i]);$(obj.pageData).append(strHTML);}
if(obj.expand==undefined){$this._render(obj);}else{obj.expand(obj);}}
this._getData=function(obj){obj.dataUrl=$this._makeUrl(obj);$.ajax({type:"get",dataType:"xml",url:obj.dataUrl,cache:false,async:false,success:function(xml){xmlData=$(xml).find("item");obj.pageCount=$(xml).find("pageCount").text();$this._showData(obj);},error:function(xml){xmlData=null;}});}
this._calculate=function(obj){if(obj.pageCount<5){this._pageIncludeTpl=obj.pageCount;}
obj.pageIndex=parseInt(obj.pageIndex);if(obj.pageIndex>obj.pageCount){obj.pageIndex=obj.pageCount;}
if(obj.pageIndex<1){obj.pageIndex=1;}
$this._start=Math.max(1,obj.pageIndex-parseInt(this._pageIncludeTpl/2));$this._end=Math.min(obj.pageCount,$this._start+this._pageIncludeTpl-1);$this._start=Math.max(1,$this._end-this._pageIncludeTpl+1);}
this._render=function(obj){$this._calculate(obj);var htmlStr="";var currpage=parseInt(obj.pageIndex);htmlStr+='<span class="paginations">';if(1!=currpage){htmlStr+='<span class="n"><a href="javascript:;" page='+(currpage-1)+' class="hi_bl"><b>上一页</b></a></span>';}
htmlStr+='<span class="p"> ';if(1!=currpage){htmlStr+='<a href="javascript:;" page="1">1</a>';}
var temp1=4;if(obj.pageCount<6){temp1=5;}
if(currpage>temp1){htmlStr+='<a href="javascript:;">...</a>';}
for(var i=$this._start;i<=$this._end;i++){if(i==currpage){htmlStr+="<em>"+i+"</em>";}else{if(i!=1&&i!=obj.pageCount){htmlStr+='<a href="javascript:;" page="'+i+'">'+i+'</a>';}}}
var temp=3;if(obj.pageCount<6){temp=5;}
if(currpage+temp<obj.pageCount){htmlStr+='<a href="javascript:;">...</a>';}
if(currpage!=obj.pageCount){htmlStr+='<a href="javascript:;" page="'+obj.pageCount+'">'+obj.pageCount+'</a>';}
htmlStr+="</span>";if(obj.pageCount!=currpage){htmlStr+='<span class="n"><a href="javascript:;" page='+(currpage+1)+' class="hi_br"><b>下一页</b></a></span>';}
htmlStr+='</span>';$(obj.pageId).html(htmlStr);var a_list=$(obj.pageId).find("a");for(var i=0;i<a_list.length;i++){a_list[i].onclick=function(){var index=this.getAttribute("page");if(index!=undefined&&index!=''){obj.pageIndex=index;if(obj.pageType=="WCM"){$this._getData(obj);}else if(obj.pageType=="APP"){var results={"pageIndex":obj.pageIndex,"pageCount":obj.pageCount};obj.ajaxData(results);if(obj.expand==undefined){$this._render(obj);}else{obj.expand(obj);}}}}}}}
var getPage=function(param){var obj={"pageType":param.pageType,"ajaxData":param.ajaxData,"pageId":param.pageId,"pageData":param.pageData,"dataUrl":param.dataUrl,"pageIndex":1,"pageCount":0,"expand":param.expand}
if(obj.pageType=="WCM"){new Page()._getData(obj);}else if(obj.pageType=="APP"){var results=obj.ajaxData();if(results!=null){obj.pageCount=results.pageCount;if(obj.expand==undefined){new Page()._render(obj);}else{obj.expand(obj);}}}}
var getValue=function(pram1){var str=window.location.search;if(str.indexOf(pram1)!=-1){var pos_start=str.indexOf(pram1)+pram1.length+1;var pos_end=str.indexOf("&",pos_start);if(pos_end==-1){return str.substring(pos_start);}else{return str.substring(pos_start,pos_end);}}else{return"";}};var ChangeTab=function(pram1,pram2,pram3,pram4){pram1.each(function(index){var $this=$(this);if(index==0){$this.addClass(pram3);pram2.eq(index).show();}
$this.click(function(){if(pram4){var $parent=$this.parent();var curIdx=$parent.index();var relURL=$this.attr("rel");$.ajax({type:"get",dataType:"html",url:relURL,cache:false,async:false,success:function(data){$this.addClass(pram3);$parent.siblings().find("a").removeClass(pram3);pram2.html(data);},error:function(xml){}});}else{var $parent=$this.parent();var curIdx=$parent.index();$this.addClass(pram3);$parent.siblings().find("a").removeClass(pram3);pram2.eq(curIdx).show().siblings().hide();}});});};var Slide=function(obj){var $root=obj.event;var $slides=$root.find('.window .slide');var $eventBtn=$root.find('.indicator');$eventBtn.delegate('li','click',function(){var $this=$(this);$slides.eq($this.index()).fadeIn().siblings().fadeOut();$this.addClass(obj.active).siblings().removeClass(obj.active);});$slides.eq(0).show().siblings().hide();$eventBtn.find('li').eq(0).addClass(obj.active).siblings().removeClass(obj.active);var next=function(){var $next=$eventBtn.find('li.'+obj.active).next();if($next.length){$next.click();}else{$eventBtn.find('li').eq(0).click();}}
if(obj.flag){var flag_hover=setInterval(next,obj.autoTime);}
$root.hover(function(){if(obj.flag){clearInterval(flag_hover);}},function(){if(obj.flag){flag_hover=setInterval(next,obj.autoTime);}});}
var ve=$.browser,isIE6=function(){return ve.msie&&(ve.version==="6.0");},access=function($this,val,fn){var name="qd_fix_"+val;if(!$this.hasClass(name)){$this.addClass(name);if(!!fn){fn(name);}}},styleHook={"auto":"0px"},oldStyle={},oldStyleFn=function($this,name){var obj=oldStyle[$this],val={},tem="";if(!obj){obj=oldStyle[$this]={};}
tem=val[name]=$this.css(name);if(isIE6){val[name]=styleHook[tem]||tem;}
$.extend(obj,val);},fixHook={},typeName=["top","bottom","left","right","center","middle"],typeNameHook={"top":"left","bottom":"left","left":"top","right":"top","center":"top","middle":"left"};$.each(typeName,function(i,val){fixHook[val]=val==="center"?function($this){access($this,val,function(){oldStyleFn($this,"margin-left");$this.css("margin-left","-"+$this.width()/2+"px");});}:val==="middle"?function($this){access($this,val,function(){oldStyleFn($this,"margin-top");$this.css("margin-top","-"+$this.height()/2+"px");});}:function($this){access($this,val);};});fixHook.abs=function($this,mar,px){oldStyleFn($this,mar);if(isIE6()){if(mar==="top"){$this.attr("style","top:expression((topwindowscroll=document.documentElement&&document.documentElement.scrollTop||document.body.scrollTop)+"+px+"+'px' )");}else{$this.get(0).style[mar]=px+"px";}}else{$this.get(0).style[mar]=px+"px";}};var fix=function(id,type,clas){var types=type.split(","),i=0,$this=$("#"+id),mar="",flag=false;fn=null;if(!$this.hasClass("qd_fix")){$this.addClass("qd_fix");}
try{for(;i<2;i++){type=types[i];fn=fixHook[type];if(!!type&&!!fn){fn($this);flag=true;continue;}
type=parseInt(type);if(isNaN(type)){continue;}
mar=flag&&types[0]||i&&"top"||"left";mar=typeNameHook[mar];if(!!mar){fixHook.abs($this,mar,type);}}}catch(e){throw new Error(e);}
if(!!clas){$this.addClass(clas);}};var fixRemove=function(id,type,clas){var types=type&&type.split(",")||typeName,i=0,l=types.length,$this=$("#"+id),css=oldStyle[$this];$this.removeClass("qd_fix");for(;i<l;i++){type=types[i];if(!!type){$this.removeClass("qd_fix_"+type);}}
if(!!clas){$this.removeClass(clas);}
$this.css(css);delete oldStyle[$this];};var Scroll=function(idobj,scrollnum,timer,hidebtn,prevclassname,nextclassname,scrolldirect,istoscroll){if(!idobj){alert("idobj is null");return false;}
idobj.productScroll({scrollnum:scrollnum,timer:timer,hidebtn:hidebtn,prevclassname:prevclassname,nextclassname:nextclassname,scrolldirect:scrolldirect,istoscroll:istoscroll});};window.TRS={Page:getPage,getValue:getValue,ChangeTab:ChangeTab,Slide:Slide,fix:fix,fixRemove:fixRemove,floatingAd:floatingAd,Scroll:Scroll,imgZoom:imgZoom,starLevel:starLevel,newsScroll:newsScroll,sidebar:sidebar,focusMap:focusMap};})(jQuery);