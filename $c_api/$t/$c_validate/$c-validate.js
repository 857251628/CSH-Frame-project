/**
 * Created by cl on 2017/3/25.
 * Jquery valide 表单验证jquery插件
 *
 */
$c.$g_input_file($c.$g_getRootPath() + "$c_api/$t/$c_validate/$c-validate.css");
(function ($) {
    /**
     * 该内部方法是在将validate以插件的形式写入jquery中
     */
    (function () {
                ...
                $this.find("[datatype]").blur(function () {
                    ...
                    if (!flag) {
                        ...
                        if (settings.rightinfo) {//设置了正确提示信息,右侧提示的方式//Set the correct message, tips on the right side
                            $.fn.validform.sn.showmsg(settings.rightinfo, settings.tiptype, {
                                obj: $(this),
                                type: 2
                            }, "hide");
                        } else {
     ...
     * @param $type  提示类型，1，为弹窗，2为右侧提示模式
     * @param $info 3，表单成功提示信息，如果不需要提示校验正确信息，只需要设置值为空 default value ""
     */
    function validForm($target, tiptype, rightinfo) {
        if (!$target) {
            throw new Error("操作对象不存在");
        }
        $target.validform({
            tiptype: tiptype,
            rightinfo: rightinfo
     ...
     */
    function submitValidForm($target, tiptype, rightinfo) {
        if (!$target) {
            throw new Error("操作对象不存在");
        }
        $target.submitvalidform({
            tiptype: tiptype,
            rightinfo: rightinfo
        });
    };
    ...
    $c.$g_add_$c_function({
        $t_validForm: validForm,
        $t_submitValidForm: submitValidForm,
        $t_resetForm: resetForm
    });
})(jQuery);
